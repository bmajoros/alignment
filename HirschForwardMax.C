...
HirschForwardMax::HirschForwardMax(int minX,int maxX,int minY,int maxY,
...
/****************************************************************
                         runPrescan()
 ****************************************************************/
void HirschForwardMax::runPrescan()
{
  WindowColumn &thisCol=frame.getThisCol(), &nextCol=frame.getNextCol();
  ...
  for(int y=lowerY+1 ; y<=upperY ; ++y) {
	...
	if(logP>bestP) { bestP=logP; bestH=h; }
      ...
      pillar[k]=bestP;
      preds[k]=bestH;
      //if(isFinite(bestP)) found=true;//###DEBUGGING
    }
  }
  //if(!found) cout<<"A "<<frame.getThisColPos()<<endl;//###DEBUGGING

  //found=false;//###DEBUGGING
  for(int j=minX+1 ; j<=maxX ; ++j) 
    {
      // SECOND INITIALIZATION STEP -- fill in bottom pillar of nextCol:
...
if(states.size()==0) INTERNAL_ERROR; // ###
	...
	for(; cur!=end ; ++cur) {
      ...
      //if(!found) cout<<"B "<<frame.getThisColPos()<<endl;
...
if(states.size()==0) INTERNAL_ERROR; // ###
	...
	for(; cur!=end ; ++cur) {
      ...
      //if(!found) cout<<"C "<<frame.getThisColPos()<<endl;//###DEBUGGING
      if(j<maxX) frame.advanceWindow();
    }
...
float HirschForwardMax::scorePath(StatePath &path)
{
