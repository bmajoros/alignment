...
void Application::processCmdLine(int argc,char *argv[]) 
{
  ...
  if(cmd.numArgs()!=5)
    throw String("\n\
sample-annotations <model.lambda> <*.fasta> <*.aln> <#samples> <outfile>\n\
...
void Application::samplingLoop()
{
  ...
  for(int i=0 ; i<numSamples ; ++i) {
    ...
    //cout<<"loading rog connections"<<endl;
    rog.loadConnections(aln);
    /*
    if(wantPrint) {
			 ...
			 int sampleNum)
{
  ...
  for(int branchIndex=0 ; branchIndex<numBranches ; ++branchIndex) {
    ...
    */
    if(!sampleBranch(branch,f,sampleNum,paths)) return false;
    /*
    if(wantPrint) {
  ...
  for(int branchIndex=0 ; branchIndex<numBranches ; ++branchIndex) {
    ...
    savePath(*paths[branchIndex],f,branch->getParentTaxon(),
	     branch->getChildTaxon(),*hmm,sampleNum);
  }
  f<<endl;
 ...
 ****************************************************************/
bool Application::sampleBranch(PhylogenyBranch *br,ostream &f,int sampleNum,
			       Vector<StatePath*> &paths)
{
  BranchAttributes *branch=
    static_cast<BranchAttributes*>(br->getDecoration());
  ...
  installFuncParse(path,f,parent,child,*hmm,sampleNum);
...
void Application::savePath(StatePath &path,ostream &f,Taxon &parent,
			   Taxon &child,BranchHMM &hmm,int sampleNum)
{
  FunctionalParse &parentParse=parent.getFunctionalParse();
  ...
  int alignmentID=(PID-1)*numSamples+sampleNum;
  ...
  for(int i=0 ; i<L ; ++i) {
    ...
    if(parentC!=prevParentC && prevParentC.isValid())
      //if(prevParentC.isBackground()) {
      if(parentC.beginsElement()) {
	parentBegin=parentPos;
	childBegin=childPos;
	beginsHere=true;
      }
      else if(parent.getBranchToParent()==NULL && parentC.isBackground() && 
	      child.whichChild()==LEFT)
	f<<parentName<<"\tREAPR\t"<<prevParentC.getElementType().getName()
	 <<"\t"<<parentBegin<<"\t"<<parentPos<<"\t.\t+\t.\talignment="
	 <<alignmentID<<"\n";
    if(childC!=prevChildC && prevChildC.isValid())
      //if(prevChildC.isBackground()) {
      if(childC.beginsElement()) {
	parentBegin=parentPos;
	childBegin=childPos;
	beginsHere=true;
      }
      else if(childC.isBackground())
	f<<childName<<"\tREAPR\t"<<prevChildC.getElementType().getName()<<"\t"
	 <<childBegin<<"\t"<<childPos<<"\t.\t+\t.\talignment="
	 <<alignmentID<<"\n";
    if(beginsHere) {
	...
	childC.getElementType().getName() : parentC.getElementType().getName();
      f<<parentName<<":"<<childName<<"\tREAPR\t"<<toString(gainLoss)
       <<"\t"<<parentBegin<<"\t"<<childBegin<<"\t.\t+\t.\talignment="
       <<alignmentID<<";factor="<<factor<<";parent="
       <<parentC<<";prevParent="<<prevParentC<<";child="<<childC
       <<";prevChildC="<<prevChildC
       <<"\n";
    }
