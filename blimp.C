/****************************************************************
 BLIMP : Binding Location Inference via Marginalized posteriors
...
protected:
  ...
  void processDir();
  float combine(Vector<float> &,Vector<float> &weights,Vector<int> &taxonIDs);
...
int Application::main(int argc,char *argv[])
{
  ...
  if(cmd.numArgs()!=4)
    throw String(
"\n\
blimp [options] <model.lambda> <target-species> <infile> <outfile>\n\
   options are:\n\
...
");
  ...
  if(!wantClass.isEmpty() && !wantForegroundDump)
    throw "option -n requires -f and filename";
  ...
  targetID=nameToTaxon[targetSpecies];
...
void Application::processDir()
{
  ...
  int targetLength=-1;
  Array1D< Vector<float> > uberSums;     // [position][i]
  Array2D< Vector<float> > joint;        // [position][taxonID][i]
  Array2D< Vector<float> > marginal;     // [position][taxonID][i]
  Vector<int> taxonIDs;
  ...
  for(; cur!=end ; ++cur) {
    ...
    */
    ...
    if(uberSums.size()<L1) {
      uberSums.resize(L1);
      targetLength=L1;
      joint.resize(targetLength,numTaxa);
      marginal.resize(targetLength,numTaxa);
    }
    for(short x=0 ; x<L1 ; ++x) {
	...
	for(; cur!=end ; ++cur) {
	  ...
	  logValues.push_back(v);
	  if(hmm->crossFunctionalType(q)==GLT_RETENTION)
	    joint[x][otherTaxonID].push_back(v);
	  marginal[x][otherTaxonID].push_back(v);
	}
	/*
	if(hmm->crossFunctionalType(q)==GLT_RETENTION) {
	  cur=entries.begin();
	  for(; cur!=end ; ++cur) {
	    Entry &entry=*cur;
	    conservation[x][otherTaxonID].push_back(entry.value);
	  }
	}
	*/
      }
  ...
  if(os) {
    ...
    for(short x=0 ; x<targetLength ; ++x) {
      float combined=combine(uberSums[x],weights,taxonIDs);
      out<<x<<"\t"<<exp(combined);
      for(int i=0 ; i<numTaxa ; ++i) {
	if(i==targetID) continue;
	if(!taxa[i].isLeaf()) continue;
	Vector<float> &j=joint[x][i], &m=marginal[x][i];
	{//if(!j.empty() && !m.empty()) {
	  float cond=sumLogProbs(j)-sumLogProbs(m);
	  if(!isFinite(cond) || isNaN(cond)) cond=NEGATIVE_INFINITY;
	  //if(joint>marginal) INTERNAL_ERROR;
	  out<<"\t"<<taxa[i].getName()<<"="<<exp(cond);
	}
      }
      out<<endl;
    }
...
float Application::combine(Vector<float> &v,Vector<float> &weights,
			   Vector<int> &taxonIDs)
{
  float combined;
  switch(combiningStrategy)
    {
    case CS_MEAN:
      combined=combine_Ave(v);
      break;
    case CS_POWER_MEAN:
      combined=combine_powerMean(v);
      break;
    case CS_WEIGHTED_POWER_MEAN:
      combined=combine_weightedPower(v,weights);
      break;
    case CS_PHYLO_LOGISTIC:
      combined=combine_phyloLogistic(v);
      break;
    case CS_PHYLO_CRF:
      combined=combine_phyloCRF(v);
      break;
    case CS_PHYLO_SPAN:
      combined=combine_phyloSpan(v,taxonIDs);
      break;
				     ...
				     Vector<int> &taxonIDs)
{
  ...
  //for(int i=0 ; i<n ; ++i) cout<<exp(X[i])<<" ";
  //cout<<"=> "<<combined<<endl;
  return log(combined);
}
