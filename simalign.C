...
int Simalign::main(int argc,char *argv[])
{
  // Process command line
  CommandLine cmd(argc,argv,"Ld:s:l:");
  if(cmd.numArgs()!=5)
    throw String(
"simalign [opts] <in.lambda> <root-length> <out.fasta> <out.maf> <out.anno>\n\
...
");
  ...
  int seed=cmd.option('s') ? cmd.optParm('s').asInt() : int(time(0));
...
/****************************************************************
                      gatherCladeMembers()
 ****************************************************************/
void Simalign::gatherCladeMembers()
{
...
/****************************************************************
                         Simalign::initBranches()
 ****************************************************************/
void Simalign::initBranches()
{
  ostream *os=NULL;
  if(shouldDumpModel) {
...
/****************************************************************
                         Simalign::generateRootSeq()
 ****************************************************************/
void Simalign::generateRootSeq()
{
...
/****************************************************************
                         Simalign::sampleResidue()
 ****************************************************************/
BOOM::Symbol Simalign::sampleResidue(const Array1D<double> &eqFreqs) 
{
  double r=Random0to1(), cumulative=0.0;
  BOOM::Symbol i;
  for(i=0 ; i<4 ; ++i) {
...
/****************************************************************
                         Simalign::evolve()
 ****************************************************************/
void Simalign::evolve(Taxon &parent)
{
...
/****************************************************************
                   Simalign::countGainLoss()
 ****************************************************************/
void Simalign::countGainLoss(StatePath &path,BranchHMM &hmm)
{
  int len=path.length();
  ...
  for(int i=0 ; i<len ; ++i) {
    ...
    if(glt==prevGLT || glt==GLT_VOID) continue;
    ++gainsLosses[glt];
    prevGLT=glt;
  }
}
...
/****************************************************************
                   Simalign::sampleChildSeq()
 ****************************************************************/
void Simalign::sampleChildSeq(BranchAttributes &branch,Taxon &parent,
			      Taxon &child,StatePath &path,BranchHMM &hmm)
{
  ...
  for(int i=0 ; i<pathLen ; ++i) {
    ...
    case PHMM_DELETE: 
      ++parentPos;
      break;
    default: throw "Simalign::sampleChildSeq() : unknown state type";
    }
  }
}
...
/****************************************************************
                   Simalign::reconstructHistory()
 ****************************************************************/
void Simalign::reconstructHistory(StatePath &statePath,PairHMM &pairHMM,
...
/****************************************************************
                    Simalign::alignmentStats()
 ****************************************************************/
void Simalign::alignmentStats(MultSeqAlignment &A,ostream &os)
{
  ...
  for(int pos=0 ; pos<L ; ++pos) {
	...
	if(!fc.isBackground() && prevFC.isBackground()) {
	  ++numPresent;
	  if(isRoot) ++rootSites;
	}
      }
...
/****************************************************************
                        Simalign::emit()
 ****************************************************************/
void Simalign::emit(const String &outfile,const String &mafFile,
		    const String &annoFile)
{
  ...
  for(int i=0 ; i<numTaxa ; ++i) {
    ...
    for(int i=0 ; i<n ; ++i) {
      ...
      String substrate=taxon.getName(), source="simalign";
