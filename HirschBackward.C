 ...
 ****************************************************************/
#include <iostream>
#include "HirschBackward.H"
#include "BOOM/Constants.H"
#include "BOOM/Exceptions.H"
using namespace std;
using namespace BOOM;

HirschBackward::HirschBackward(int minX,int maxX,int minY,int maxY,
  ...
  for(int j=maxX-1 ; j>=minX ; --j) 
    {
      ...
      frame.zeroOut(pillar);//pillar.setAllTo(LOG_0);
      for(STATE k=1 ; k<numStates ; ++k) {
	float bestP=LOG_0;
	const Array1D<STATE> &succQ=hmm.getSuccessors(k);
	int numSuccQ=succQ.size();
...
/****************************************************************
                         runPrescan()
 ****************************************************************/
void HirschBackward::runPrescan()
{
  WindowColumn &thisCol=frame.getThisCol(), &nextCol=frame.getNextCol();
